...
> **Випуск `1.2.0`.** Працюйте безпосередньо в поточній сесії Codex з 18 MCP-інструментами та 13 спеціалізованими skills. Codex керує розмовою й виконанням; APEXREST виконує операції Oracle/APEX. Програмне очікування завдань, пакетні метадані, стисла перевірка проєкту й цільова довідка скорочують рутинні виклики. Зміни й межі доказів — у [примітках до випуску](docs/release-notes.uk.md). Незалежні інструменти; це не офіційний продукт Oracle чи OpenAI.

Опишіть зміну в чаті Codex або використовуйте `$apexrest-work`. Реалізація починається в тому самому контексті без виклику запуску плагіна. Дивіться [сценарій чату](docs/chat-workflow.uk.md) та [інтеграцію Codex](docs/codex-integration.uk.md). Для перевірки APEX оберіть **Codex in-app browser / External system browser** у Settings.
...
Відкрийте `$apexrest-panel` для налаштувань проєкту й з’єднань, фактичних завдань Oracle/APEX і діагностики в Codex. Консольне подання — `apexrest panel tui`. Дивіться [панель розробки](docs/panel.uk.md).

[Каталог компонентів](docs/component-catalog.uk.md) дає офлайн-пошук серед 109 сімейств, параметри з контекстом і 138 перевірених компілятором рецептів APEXlang; один непідтриманий рецепт має явний статус unresolved. [Список компонентів і приклади](https://apex.oracle.com/ut) доступні онлайн. Використовуйте наявні довідкові інструменти або CLI з `--corpus components`; докази компіляції, SQL, імпорту й браузерної перевірки розділено.

Випуск 1.2.0 додає окремий офлайн-[каталог патернів](docs/pattern-catalog.uk.md): 58 готових UX-патернів і 84 рецепти, з яких 69 перевірено компілятором, а 15 мають явні прогалини. Огляд охоплює 150 сторінок джерел і 818 рішень щодо варіантів. Шукайте через `--corpus patterns`; використовуйте новий skill `$apexrest-pattern-catalog`, щоб додати перевірені патерни з іншого застосунку APEX. Готовність за компілятором не означає перевірку SQL, імпорту чи браузера.
...
| ------------------------ | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Де виконуються задачі    | На вашому комп’ютері; він має залишатися ввімкненим, поки виконуються операції. | У хмарному контейнері; домашній комп’ютер може бути вимкнений.                 |
...
[Посібник Cloud](docs/codex-cloud.uk.md) описує CLI + ORDS у налаштованому контейнері. Він не підтверджує нативне виявлення плагіна/MCP чи підтримку панелі у Cloud. Локальні й підключені докази зберігають свої версії та межі.

## Встановлення з npm

Використовуйте Node 24 LTS (підтримуваний діапазон: Node 24–26). Встановіть [apexrest](https://www.npmjs.com/package/apexrest) з npm. Версія цього пакета — `1.2.0`; наведені нижче команди використовують `latest`. Стан розповсюдження й перевірок — у [примітках до випуску](docs/release-notes.uk.md).
...
Щоб зафіксувати цю версію, використовуйте `npm install -g apexrest@1.2.0`. npm містить готове середовище виконання, тому копія Git-репозиторію чи локальне збирання не потрібні. Наведене нижче встановлення з репозиторію також доступне.
...
> Скористайся APEXREST, щоб перенести наявний застосунок із налаштованого тестового середовища в новий проєкт. Додай дашборд продажів із реальними показниками бази даних. Збережи наявні сторінки, спільні компоненти й автентифікацію, потім імпортуй і перевір зміну.
