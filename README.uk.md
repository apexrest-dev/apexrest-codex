...
Опишіть реалізацію в чаті Codex: `$apexrest-work` запускає обраного єдиного агента чи команду, відкриває вбудовану панель і повертає перевірений результат у ту саму розмову. Моделі й рівні міркування обирає Auto; вебформа й ручна конфігурація моделей не потрібні. Дивіться [сценарій із чату та моделі Auto](docs/chat-workflow.uk.md). Команда зберігає окремих розробників, обов’язкове рев’ю коду менеджером, незалежний QA та фінальне рев’ю менеджером. Дивіться [API команди та умови схвалення](docs/team.uk.md) й [аудит коду Codex](docs/codex-integration.uk.md).

У Settings можна обрати **Single agent / Agent team** та **Codex in-app browser / External system browser** для перевірок APEX. Обидва режими мають живий dashboard. [Налаштування й межі перевірки](docs/work-modes.uk.md).
...
*Справжній знімок вбудованого браузера Codex. Ізольований приклад із кодом перевіряє виконання команди; він не показує імпорт Oracle.*
