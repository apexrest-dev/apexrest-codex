...
Завдання реалізації запускаються через `$apexrest-team`: окремі розробники, обов’язкове рев’ю коду менеджером, незалежна QA та фінальне рев’ю менеджером. Дивіться [API команди та умови схвалення](docs/team.uk.md) й [аудит коду Codex](docs/codex-integration.uk.md).
