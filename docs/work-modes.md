...
Use a fresh UUID for `REQUEST_UUID`. `--execution-mode team|single` and `--browser-mode codex|external` explicitly override defaults for a run. Omitting them honors saved settings. The MCP equivalents are `apexrest_panel_action`, `apexrest_work_start` and `apexrest_browser_open`. The browser opener accepts an explicit configured environment, not arbitrary URLs or shell commands.
