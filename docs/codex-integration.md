...
For a future status UI, use one authoritative snapshot of team phase, review decisions and APEX operation jobs. First verify a generated status artifact in a native Codex file panel, or prove a minimal MCP UI resource in the actual Codex desktop; retain native conversational/tool output in Codex CLI. Polling the status tool is available without the hosted-app event API. Do not claim a native desktop panel until actual rendering and progress updates are observed. No panel or Codex fork was implemented in this audit.

The mandatory guarantee applies to executions started through `apexrest_team_start`: the controller cannot return `completed` without its review gates. A plugin skill and default prompt do not globally intercept arbitrary manual edits, shell commands or independent low-level tool calls in Codex. Such work has no team approval. This distinction is necessary even for a Codex-only product.
