...
var require_utils = __commonJS({
  "node_modules/fast-uri/lib/utils.js"(exports, module) {
    ...
    function percentEncodeNonAscii(cp3) {
      if (cp3 < 2048) {
        return BYTE_HEX[192 | cp3 >> 6] + BYTE_HEX[128 | cp3 & 63];
      }
      if (cp3 < 65536) {
        return BYTE_HEX[224 | cp3 >> 12] + BYTE_HEX[128 | cp3 >> 6 & 63] + BYTE_HEX[128 | cp3 & 63];
      }
      return BYTE_HEX[240 | cp3 >> 18] + BYTE_HEX[128 | cp3 >> 12 & 63] + BYTE_HEX[128 | cp3 >> 6 & 63] + BYTE_HEX[128 | cp3 & 63];
    }
    ...
    function isEscapeSafe(cp3) {
      return cp3 >= 48 && cp3 <= 57 || cp3 >= 65 && cp3 <= 90 || cp3 >= 97 && cp3 <= 122 || cp3 === 42 || cp3 === 43 || cp3 === 45 || cp3 === 46 || cp3 === 47 || cp3 === 64 || cp3 === 95;
    }
...
async function runSqlclMcp(request) {
  ...
  const call = async (name, args) => {
    const result = await client.callTool({ name, arguments: args }, void 0, options());
    text(result);
    return result;
  };
...
var OracleAdapter = class {
  ...
  async settings() {
    ...
    return {
      ...
      javaHome: process.env.APEXREST_JAVA_HOME ?? (state.java ? path.dirname(path.dirname(state.java)) : process.env.JAVA_HOME)
    };
  }
...
`,
  ...
  async validate(source, signal) {
    ...
    const stage = await this.stage(), copy = path.join(stage, "application");
    ...
    return {
      status: "passed",
      compiler,
      mmd: JSON.parse(await readFile(path.join(source, ".apex/apexlang.json"), "utf8")),
  ...
  async exportApplication(env, connection, format = "APEXLANG") {
    ...
    if (!Object.keys(files).length || format === "SQL" && !Object.keys(files).some((f) => f.endsWith(".sql")))
...
function resourceRoot() {
  return process.env.APEXREST_RESOURCES ?? path2.resolve(path2.dirname(fileURLToPath(import.meta.url)), "../resources");
}
async function projectInit(directory, template, alias) {
  ...
  await createFile(path2.join(root, "apexrest.json"), JSON.stringify(config, null, 2) + "\n");
  await createFile(
  ...
  for (const dir of [
    ...
    await mkdir2(path2.join(root, dir), { recursive: true });
  await cp2(
    path2.join(resourceRoot(), "toolchains/toolchain.lock.json"),
    path2.join(root, config.toolchain.lockFile),
  ...
  if (template !== "existing-app") {
    ...
    if (template === "customer-crm") {
      await cp2(path2.join(resourceRoot(), "templates/customer-crm/project"), root, { recursive: true });
      await cp2(
        path2.join(resourceRoot(), "templates/customer-crm/apex-overlay"),
        path2.join(root, config.application.sourceDir),
        { recursive: true }
      );
      const { readFile: readFile2 } = await import("node:fs/promises");
      const file = path2.join(root, config.application.sourceDir, "shared-components/lists.apx");
