...
var require_yauzl = __commonJS({
  "node_modules/yauzl/index.js"(exports) {
    ...
    function openPromise(path4, options2) {
      return new Promise((resolve, reject) => {
        open(path4, { ...options2, lazyEntries: true }, function(err, zipfile) {
...
import { cp, mkdir as mkdir2, mkdtemp, rename, chmod, statfs, realpath } from "node:fs/promises";
...
async function extractArchive(file, target, type, allowLinks = false) {
  ...
  if (type === "tar.gz") {
      ...
      onReadEntry: (entry) => {
        ...
        try {
          ...
          if (entry.type === "SymbolicLink" || entry.type === "Link") {
            ...
            const linkTarget = entry.type === "Link" ? entry.linkpath : path2.posix.join(path2.posix.dirname(name), entry.linkpath);
            if (path2.isAbsolute(entry.linkpath))
    ...
    for (const entry of links) {
      ...
      else await symlink(path2.relative(path2.dirname(destination), source), destination);
    }
    return;
  }
    ...
    import_yauzl.default.open(file, { lazyEntries: true, validateEntrySizes: true, strictFileNames: true }, (error, zip) => {
      ...
      zip.on("entry", (entry) => {
        void (async () => {
          ...
          if (name.endsWith("/")) await mkdir(destination, { recursive: true, mode: 448 });
          else {
...
async function runtimeState(home = managedHome()) {
  const file = path3.join(home, "runtime.json");
...
async function findExecutable(name) {
  for (const directory of (process.env.PATH ?? "").split(path3.delimiter))
    for (const suffix of process.platform === "win32" ? ["", ".exe", ".cmd"] : [""]) {
      const candidate = path3.join(directory, name + suffix);
...
var ToolchainService = class {
  ...
  async plan(r) {
    ...
    const lock = parse(
      lockSchema,
      await readJson(path3.join(resourceRoot(), "toolchains/toolchain.lock.json"))
    );
    const home = path3.resolve(r.home ?? managedHome()), cache = path3.resolve(r.cacheDir ?? path3.join(home, "cache"));
    const artifacts = lock.artifacts.filter((a) => a.os === platform.os && a.arch === platform.arch);
    const steps = await Promise.all(
      artifacts.map(async (artifact) => {
        const candidate = artifact.id === "node" ? process.execPath : artifact.id === "sqlcl" ? process.env.APEXREST_SQLCL ?? await findExecutable("sql") : process.env.APEXREST_JAVA_HOME ? path3.join(
        ...
        const destination = path3.join(home, "toolchains", artifact.id, artifact.version);
        return {
          artifact,
          destination,
          reuse,
          action: reuse ? "reuse" : await exists(path3.join(destination, artifact.executable)) ? "verify" : "download-install",
  ...
  async apply(r) {
    ...
    await mkdir2(plan.home, { recursive: true, mode: 448 });
    ...
    return withLock(path3.join(plan.home, "toolchain.lock"), async () => {
      ...
      for (const step of plan.steps) {
        ...
        const executable = step.reuse ?? path3.join(step.destination, artifact.executable);
        if (!await exists(executable)) {
          const file = await download(artifact, plan.cache, r.offline);
          await mkdir2(path3.dirname(step.destination), { recursive: true });
          const staging = await mkdtemp(step.destination + ".staging-");
          await extractArchive(file, staging, artifact.type, artifact.id !== "sqlcl");
          if (!await exists(path3.join(staging, artifact.executable)))
            throw new Fault(
        ...
        const javaHome = state.java ? path3.dirname(path3.dirname(state.java)) : void 0;
        const result = await runProcess({
          executable,
          args: artifact.id === "node" ? ["--version"] : ["-version"],
          cwd: plan.home,
        ...
        await writeJson(path3.join(plan.home, "runtime.json"), state);
      }
      if (!r.skipBrowser && state.node) {
        const browserHome = path3.join(plan.home, "playwright", plan.playwright), browserCache = path3.join(plan.home, "browsers");
        await mkdir2(browserHome, { recursive: true });
        await cp(
          path3.join(resourceRoot(), "playwright/package.json"),
          path3.join(browserHome, "package.json")
        );
        await cp(
          path3.join(resourceRoot(), "playwright/package-lock.json"),
          path3.join(browserHome, "package-lock.json")
        );
        const npm = path3.resolve(
          path3.dirname(state.node),
          process.platform === "win32" ? "node_modules/npm/bin/npm-cli.js" : "../lib/node_modules/npm/bin/npm-cli.js"
        );
        const env = {
          ...process.env,
          PLAYWRIGHT_BROWSERS_PATH: browserCache,
          npm_config_cache: path3.join(plan.cache, "npm"),
          PATH: path3.dirname(state.node) + path3.delimiter + (process.env.PATH ?? "")
        };
        if (!await exists(path3.join(browserHome, "node_modules/@playwright/test/cli.js"))) {
        ...
        const cli = path3.join(browserHome, "node_modules/@playwright/test/cli.js");
        if (!r.offline) {
          const installed = await runProcess({
            executable: state.node,
