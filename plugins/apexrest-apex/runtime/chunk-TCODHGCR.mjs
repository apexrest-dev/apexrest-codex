...
function apexlangCatalogue(records) {
  ...
  for (const record2 of records) {
    if (!record2 || typeof record2 !== "object") throw new Error("Invalid APEXlang record");
    if (!["template", "contract", "guide"].includes(record2.kind)) continue;
    if (typeof record2.family !== "string") continue;
    const [root, name] = record2.family.split("/");
    const group = groups.get(root);
    if (!group || root !== "buttons" && (!name || name.startsWith("_"))) continue;
    if (typeof record2.id !== "string" || typeof record2.version !== "string")
    ...
    entry.ids.add(record2.id);
    versions.add(record2.version);
  }
