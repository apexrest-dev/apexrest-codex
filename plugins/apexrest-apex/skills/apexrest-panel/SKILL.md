...
Single agent is always the default. Enable `multiAgentEnabled` only on an explicit user request to change Settings. A task launch override cannot enable a team. Existing legacy `executionMode: team` values without explicit opt-in resolve to single.
